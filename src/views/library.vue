  ...
  <div id="library" class="text-wrapper">
    ...
    <van-row>
      ...
      <van-col span="24" v-show="!ifLoading2">
        ...
        >
          <van-cell v-for="item in list" :key="item" :title="item.title" value="查看馆藏" :label="item.author" is-link
                    @click="showPopup(item.marcRecNo)">查看馆藏
          </van-cell>
          <van-popup v-model:show="show" round position="bottom" :style="{ height: '50%' }"
                     v-html="showLocation"></van-popup>
